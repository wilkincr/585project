...
class PPOTrainer(ABC):
    ...
    def fit(
        ...
        # START OF KEVIN PROFILING
        log_dir = "./logs"  # Log directory for TensorBoard

        # PyTorch Profiler
        profiler = profile(
            activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],  # Capture both CPU and GPU events
            on_trace_ready=tensorboard_trace_handler(log_dir),         # Save results to TensorBoard
            # with_stack=True,                                           # Include stack traces
            record_shapes=True,                                        # Record tensor shapes
            profile_memory=True,                                       # Enable memory profiling
            with_flops=True
        )
        # END OF KEVIN PROFILING
        profiler.start()
